          ...
          <thead class="table-light">
            <tr>
              ...
              <th class="th-sortable" @click="sort('nombre')">
                Nombre (clave) <i :class="['bi', sortIcon('nombre'), 'sort-icon ms-1', { 'is-active': sortKey === 'nombre' }]"></i>
              </th>
              <th class="th-sortable" @click="sort('modulo')">
                Módulo <i :class="['bi', sortIcon('modulo'), 'sort-icon ms-1', { 'is-active': sortKey === 'modulo' }]"></i>
              </th>
              <th class="th-sortable" @click="sort('accion')">
                Acción <i :class="['bi', sortIcon('accion'), 'sort-icon ms-1', { 'is-active': sortKey === 'accion' }]"></i>
              </th>
    ...
    >
      ...
      <template #footer>
        ...
        <button class="btn btn-primary" type="button" :disabled="isSaving" @click="save">
          ...
          <i v-else class="bi bi-check-lg me-1"></i>{{ isSaving ? 'Guardando…' : editingId ? 'Guardar cambios' : 'Crear permiso' }}
        </button>
      </template>
    </AppModal>
